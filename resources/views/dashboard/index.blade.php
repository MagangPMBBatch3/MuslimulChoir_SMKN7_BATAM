...
<x-layouts.main title="Dashboard">
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 p-5 rounded-xl shadow text-white flex items-center">
                <i class='bx bx-folder text-4xl mr-4'></i>
                <div>
                    <h3 class="text-sm font-medium opacity-80">Total Proyek</h3>
                    <p class="text-3xl font-bold mt-1">{{ $totalProyek }}</p>
                </div>
            </div>

            <div class="bg-gradient-to-r from-emerald-600 to-emerald-800 p-5 rounded-xl shadow text-white flex items-center">
                <i class='bx bx-user text-4xl mr-4'></i>
                <div>
            ...
            <div class="bg-gradient-to-r from-violet-600 to-violet-800 p-5 rounded-xl shadow text-white flex items-center">
                <i class='bx bx-time-five text-4xl mr-4'></i>
                <div>
                    <h3 class="text-sm font-medium opacity-80">Lembur</h3>
                    <p class="text-3xl font-bold mt-1">{{ $totalLembur }}</p>
                </div>
            </div>

            <div class="bg-gradient-to-r from-rose-600 to-red-700 p-5 rounded-xl shadow text-white flex items-center">
                <i class='bx bx-message-dots text-4xl mr-4'></i>
                <div>
                    <h3 class="text-sm font-medium opacity-80">Pesan</h3>
                    <p class="text-3xl font-bold mt-1">{{ $totalPesan }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-slate-800 text-gray-200 p-6 rounded-xl shadow">
                <h3 class="text-lg font-bold text-white mb-4 flex items-center">
                    <i class="bx bx-line-chart mr-2"></i> Aktivitas Mingguan
                </h3>
            ...
            <div class="bg-slate-800 text-gray-200 p-6 rounded-xl shadow">
                <h3 class="text-lg font-bold text-white mb-4 flex items-center">
                    <i class="bx bx-envelope mr-2"></i> Pesan Terbaru
                </h3>
                <ul class="divide-y divide-gray-200">
                    @forelse($pesanTerbaru as $pesan)
                        <li class="py-3">
                            <p class="font-semibold">{{ $pesan->user->name }}</p>
                            <p class="text-sm text-gray-400">{{ $pesan->isi }}</p>
                            <p class="text-xs text-gray-500">{{ $pesan->tgl_pesan->format('d M Y H:i') }}</p>
                        </li>
                    @empty
        ...
        <div class="bg-slate-800 p-6 rounded-xl shadow">
            <h3 class="text-lg font-bold text-white mb-4 flex items-center">
                <i class="bx bx-briefcase mr-2"></i> Daftar Proyek
            </h3>
